...
import { addContactToExtensionLeadAction } from "@/actions/extension-leads";
import { useToast } from "@/hooks/use-toast";

export function AddExtensionLeadContactDialog({ leadId }: { leadId: string }) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const result = await addContactToExtensionLeadAction(leadId, formData);

        setIsPending(false);
        if (result.success) {
            toast({
                title: "Contact added successfully",
                description: "The new contact has been added to this draft lead.",
            });
            setOpen(false);
            ...
            <DialogContent className="sm:max-w-[500px] rounded-3xl border-2 shadow-2xl">
                <form onSubmit={handleSubmit}>
                    <DialogHeader className="mb-6">
                        ...
                        <p className="text-muted-foreground">Draft another person you want to contact at this company.</p>
                    </DialogHeader>
